...
function map_assessment_admin_page() {
    ...
    wp_enqueue_script('map-assessment-admin-scripts', plugin_dir_url(__FILE__) . 'map-assessment-admin-scripts.js', ['jquery', 'leaflet-js', 'leaflet-editable'], null, true);
    ...
    wp_localize_script('map-assessment-admin-scripts', 'mapAssessmentAjax', $script_data);
...
// AJAX handler for creating a question
function map_assessment_create_question() {
    ...
    $result = $wpdb->insert(
        $table_name,
        [
            'question_text' => sanitize_text_field($question_data['text']),
            'start_point' => $question_data['startLat'] . ',' . $question_data['startLng'],
            'end_point' => $question_data['endLat'] . ',' . $question_data['endLng'],
        ]
    );

    if ($result === false) {
        wp_send_json_error(['message' => __('Error creating question', 'map-assessment')]);
    } else {
        wp_send_json_success(['message' => __('Question created successfully', 'map-assessment')]);
    }
}
